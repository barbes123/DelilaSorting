...
void DelilaPreSorting::Read_ELIADE_LookUpTable() {
...
void DelilaPreSorting::Read_TimeAlignment_LookUpTable() {
...
void DelilaPreSorting::Read_TimeAlignment_Trigger() {
...
void DelilaPreSorting::Read_Confs() {
...
void DelilaPreSorting::Print_ELIADE_LookUpTable()
{
...
void DelilaPreSorting::Print_TimeAlignment_LookUpTable()
{
...
void DelilaPreSorting::Print_TimeAlignment_Trigger_LookUpTable()
{
...
float DelilaPreSorting::CalibDet(float val, int ch)
{
...
void DelilaPreSorting::Begin(TTree * tree)
{
...
//   double beta = ((TObjString*) toks->At(2))->GetString().Atof();
  ...
  TObjArray *branches = (TObjArray*)fChain->GetListOfBranches();
  int N(branches->GetEntries());
  
//   https://cpp.hotexamples.com/examples/-/TTree/GetListOfBranches/cpp-ttree-getlistofbranches-method-examples.html
  for(int jj = 0; jj < N; jj++) {
      TString name = fChain->GetListOfBranches()->At(jj)->GetName();
       std::cout << "branch: "<<jj<<" " <<" N "<< N <<" "<<name << std::endl;
       if (name=="Mod") std::cout << "bingo \n";
  };
...
void DelilaPreSorting::SlaveBegin(TTree * /*tree*/)
{
...
//    mTimeCalib = new TH2F("mTimeCalib", "mTimeCalib", 10000, 0, 10000, 2e3, -1e6, 1e6);
  ...
  mTimeCalibBGO = new TH2F("mTimeCalibBGO", "mTimeCalibBGO", max_domain, 0, max_domain, 1e3, -1e4, 1e4);
...
Bool_t DelilaPreSorting::Process(Long64_t entry)
{
...
void DelilaPreSorting::FillOutputTree(){
...
void DelilaPreSorting::TreatDelilaEvent()
{
...
void DelilaPreSorting::SlaveTerminate()
{
...
void DelilaPreSorting::Terminate()
{
...
int DelilaPreSorting::GetCoincTimeCorrection(int dom1, int dom2)
{
...
//  int coin_id = dom1*100+dom2;
 ...
 int DelilaPreSorting::GetCoinc_det_def(int det_def1, int det_def2)
 {
     int id=0;
...
int DelilaPreSorting::GetCoincID(int dom1, int dom2){
    int id=0;
...
void DelilaPreSorting::PrintDelilaEvent(TDelilaEvent &ev_)
{
...
void DelilaPreSorting::TreatLaBrSingle()
{
    
}

void DelilaPreSorting::TreatHpGeSingle()
{
    
}

void DelilaPreSorting::TreatNeutronSingle()
{
...
void DelilaPreSorting::TreatSolarSingle()
{
  hDelila[DelilaEvent.det_def]->Fill(DelilaEvent.EnergyCal); 
}

void DelilaPreSorting::TreatACS()
{
//     int gate = 100;
    double_t time_diff_cs = 0;
    int cs_dom = DelilaEvent.cs_domain;
    
    int gate = coinc_gates[35];
