...
void Delila_selector_elifant(UInt_t first_run=195,  UInt_t last_run=195, UInt_t vol0=1, UInt_t vol1=1, UInt_t numberofevents=0){
...
//         options<<run<<","<<vol<<","<<beta<<","<<0<<","<<"0";
	       ...
	       std::cout<<"I will start DelilaSelectorElifant with the options: "<<options.str().c_str()<<std::endl;
        	if (numberofevents == 0){
        	        ch->Process("~/DelilaSorting/DelilaSelectorElifant.C+",options.str().c_str());
                }
                else {ch->Process("~/DelilaSorting/DelilaSelectorElifant.C+",options.str().c_str(),numberofevents);};    
		};
             };
      };
}
